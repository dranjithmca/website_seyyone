<?php
/**
 * Template Name: Software Solutions Template
            ...
            <div class="rts-case-studies-area rts-section-gapBottom mt_sm--30">
                <div class="container">
                    <div class="row g-80 mt--0">
                        ...
                        if ($software_services->have_posts()) :
                            $delay_counter = 0;
                            while ($software_services->have_posts()) : $software_services->the_post();
                                $post_id = get_the_ID();
                                $title = get_the_title();
                      ...
                      <div class="col-lg-4 col-md-6 col-sm-12 wow fadeInUp" data-wow-offset="120"<?php echo $delay_attr; ?> id="<?php echo esc_attr($service_id); ?>">
                        ...
                        <div class="col-12">
                            <div style="text-align: center; padding: 60px 20px; background: #f8f9fa; border-radius: 15px; border: 2px dashed #dee2e6;">
                                ...
                                <h3 style="color: #495057; margin-bottom: 15px;">No Software Services Found</h3>
                                <p style="color: #6c757d; margin-bottom: 25px;">Add your first software service to get started.</p>
                                <?php if (current_user_can('edit_posts')) : ?>
                                    <a href="<?php echo admin_url('post-new.php?post_type=software_service'); ?>" class="rts-btn btn-primary">
                                        <i class="fa fa-plus" style="margin-right: 8px;"></i>Add New Service
                                    </a>
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <div class="cta-one-wrapper">
                    ...
                    <div class="left-area">
                        ...
                        <p class="wow fadeInUp" data-wow-delay=".3s">
                            Schedule a no-obligation consultation to discuss your unique needs and how Seyyone can elevate your business technology.
                        </p>
...
<!-- Simplified Modal Script - No Features Processing -->
<script>
...
function openModal(serviceType) {
    ...
    // Show loading
    //document.getElementById('modalContent').innerHTML = '<div style="text-align: center; padding: 40px;"><p>Loading...</p></div>';
    ...
    fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
        ...
        body: new URLSearchParams({
            'action': 'get_software_service_by_id',
            'service_id': serviceType,
            'nonce': '<?php echo wp_create_nonce('seyyone_nonce'); ?>'
        })
    })
    ...
    .catch(error => {
        ...
        document.getElementById('modalContent').innerHTML = '<div style="text-align: center; padding: 40px;"><p style="color: #dc3545;">Error loading content. Please try again.</p></div>';
    });
}
...
modal.onclick = (e) => {
 ...
 <!-- Your Modal CSS - Only Font Size Increases -->
  <!-- Your Modal CSS - Bigger Fonts -->
